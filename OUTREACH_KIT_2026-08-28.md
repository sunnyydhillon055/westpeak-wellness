# Outreach kit — every send is written; the owner adds a name and presses go

**Written:** 28 August 2026. Items 32–36 and 41 of `FIFTY_MORE_2026-08-28.md`.
Everything below is copy-paste ready and BCACC-compliant: no outcome claims, no
testimonials, no statistics invented for effect. Facts about the practice come from
the reference block in `OFFSITE_KIT_2026-08-27.md`; do not vary the wording of the
business name.

> **One rule for all outreach:** every message offers something useful to the
> recipient's audience, not a favour to this practice. Link earned that way survives
> editor turnover and algorithm updates; link begged for does not.

---

## 1 · Standing press bio (for journalist requests — Qwoted, press queries, direct)

> [COUNSELLOR NAME], MA, RCC, is a Registered Clinical Counsellor (BCACC #20111)
> running Westpeak Wellness, a fully virtual counselling practice serving all of
> British Columbia in English and Punjabi. Practice areas include anxiety, trauma
> and EMDR, couples work (Gottman-trained), and South Asian mental health. the counsellor can
> comment on: access to counselling outside BC's major cities, therapy in Punjabi
> and heritage languages, the 2027 regulation of psychotherapy in BC, and workplace
> mental-health topics (stress leave, return-to-work).
> Verification: bc-counsellors.org register, #20111. Contact: info@westpeakwellness.com.

**Rules for replies to journalist queries:** answer the actual question in the first
two sentences; no client anecdotes, ever, including anonymised ones; offer one
checkable fact the journalist can keep even if the quote dies.

## 2 · Pitch: the 2027 regulation change (The Tyee / Daily Hive / Fraser Valley Current)

Subject: **In 2027, "counsellor" finally becomes a regulated title in BC — most people have no idea**

> Hi [name] — right now, anyone in BC can legally call themselves a counsellor or
> therapist: no degree, no oversight, no complaints process. That ends on
> 29 November 2027, when psychotherapy comes under the College of Health and Care
> Professionals of BC. Almost nobody outside the profession knows this is coming,
> and the practical questions write themselves: how do you check a counsellor
> *today*, what changes for people already in therapy, and what happened in the
> provinces that regulated first?
>
> I'm a Registered Clinical Counsellor in the Fraser Valley and can walk through it
> in plain language, including the two-minute register check anyone can do now.
> A explainer on my site covers the ground if useful background helps:
> westpeakwellness.com/resources/what-is-a-registered-clinical-counsellor
>
> — [COUNSELLOR NAME], MA, RCC (BCACC #20111)

## 3 · University outreach (campus counselling-resource pages)

Targets: student-services / counselling "more resources" pages at UFV, TRU, UNBC,
VIU, KPU, SFU, UVic, and the student unions' health-plan pages.

Subject: **A free BC student mental-health resource page for your links list**

> Hi — I maintain a plain-language guide to every mental-health support a BC
> post-secondary student already has: Here2Talk, campus services, student-plan
> coverage, and the low-cost options when those run out. No paywall, no email
> gate, updated dates shown:
> westpeakwellness.com/resources/student-mental-health-supports-bc
>
> If your resources page links out to student-facing material, it may be a fit.
> Happy to have it reviewed for accuracy by your team first — corrections welcome.

## 4 · Community organisations (CMHA branches, divisions of family practice, settlement agencies)

Same shape as §3, swapping the offered page:
- CMHA branches / divisions of family practice → `/resources/bc-crisis-and-support-directory` and the work-and-money cluster (`/resources/workplace-mental-health-bc`)
- Settlement agencies (PICS, DIVERSEcity, Options, Archway) → `/services/punjabi-counselling` and `/punjabi` — note both are informational-first, and the Punjabi-language page carries no booking pressure.

Line to include for settlement agencies:

> The practice runs in English and Punjabi, virtually across all of BC — which for
> Punjabi speakers outside the Lower Mainland is often the only in-language option.
> If your client-facing resource lists include counselling options, we'd be glad to
> be checked out (BCACC register #20111) and listed — or simply to have the free
> Punjabi-language pages linked, with no referral relationship implied.

## 5 · South Asian broadcast (RED FM 93.1, Connect FM 91.5)

Ask: a short Punjabi-language segment on what counselling is and how to check a
counsellor's registration — service journalism, not advertising.

> ਸਤ ਸ੍ਰੀ ਅਕਾਲ — I'm a Registered Clinical Counsellor (BCACC #20111) practising in
> Punjabi and English across BC. I'd like to offer a short call-in-friendly segment
> answering the questions families actually ask: what happens in counselling, is it
> confidential from family, what it costs and what benefits cover, and how to verify
> any counsellor in two minutes. No client stories, no promotion beyond the name —
> the register check works for finding any counsellor, not just me.

(**Note:** the on-air conversation is the counsellor's own Punjabi — the fluent-review rule
applies to *published site copy*, not to the counsellor speaking.)

## 6 · GP / clinic referral letter (print `/refer`, attach this cover note)

Targets: family practices and UPCCs in Abbotsford, Mission, Chilliwack, Langley, Surrey.

> Re: counselling referrals for patients who need somewhere concrete
>
> Dr. [name] / Clinic team — for patients who would benefit from counselling and
> ask "where do I actually go?", Westpeak Wellness may be a useful pointer:
> Registered Clinical Counsellor (BCACC #20111, verifiable), virtual across BC,
> evening availability, sessions in English or Punjabi, published fees, free
> 15-minute consultation, no referral required. The practice page for clinicians —
> westpeakwellness.com/refer — covers scope honestly, including what is NOT seen
> here (no diagnosis, no prescribing, not a crisis service) and coordination with
> your care via patient consent.
>
> One page for patients worth knowing exists either way:
> westpeakwellness.com/resources/low-cost-counselling-bc — the free and low-cost
> options, including ones that are not this practice.

## 7 · Send log

| Date | Target | Item | Sent by | Response |
|---|---|---|---|---|
| | | | | |
