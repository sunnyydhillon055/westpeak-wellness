...
export async function GET(req: NextRequest) {
  ...
  if (!gate.ok) {
    await noteCronRefusal('booking-mail', req, gate.why);
  ...
  /* Every other job is checked from here, because this one runs every two
     ...
     It sends nothing — the verdict is rendered in /admin; see the note in
     lib/cron-health.ts for why the alert mail was withdrawn. `selfJob` lets
     it tell "the jobs stopped" from "the store stopped", which is the fault
     that actually occurred. */
  const watchdog = dry
    ? { problems: [] as string[] }
    : await runCronWatchdog({ selfJob: 'booking-mail' });
  if (watchdog.problems.length) {
    console.warn('[cron-watchdog] not running:', watchdog.problems.join(', '));
  }
  if (!run.ok) {
