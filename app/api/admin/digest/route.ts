...
import { isTestSubmission, looksHuman } from '@/lib/inbound-quality';
...
/* The test and bot filters moved to lib/inbound-quality.ts on 17 Sep 2026,
   when the reply-time watch turned out to need the same judgement and did not
   have it. Same rules, one copy, used by both. */
...
export async function POST(req: Request) {
  ...
  const items = book.items
    .filter((it) => !isTestSubmission(it) && looksHuman(it))
